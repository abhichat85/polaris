...
import type { ToolOutput } from "../core/tool-types"
import type { VerifyResult } from "../core/verify-types"
import type {
  AgentCheckpoint,
  AgentDoneStatus,
  AgentSink,
...
import type { IToolExecutor } from "../core/tool-types"
import type { StreamMonitor } from "../core/stream-monitor"
import type { StreamAlert } from "../core/telemetry"
...
/**
 * Default compaction threshold in tokens. When the running token total
 * crosses this value and a compactor is wired, the runner auto-compacts.
 */
...
export interface AgentRunnerDeps {
  ...
  sink: AgentSink
  /** Tool definitions to pass to the adapter. Caller provides. */
  tools: ToolDefinition[]
  /** Default system prompt. Caller provides. */
   ...
   * the token total crosses the compaction threshold, replaces
   * `state.messages` with the handoff artifact, and continues. When
   * absent, the existing behaviour (hard-fail at maxTokens) applies.
   */
  compact?: CompactFn
  /**
   * Compaction threshold in tokens. Defaults to DEFAULT_COMPACTION_THRESHOLD
   * (100_000) when not provided.
   */
   ...
   * canonical system prompt. Defaults to `defaultSystemPrompt`.
   */
  systemPrompt?: string
  /**
  ...
  /**
   * Phase 2 — optional StreamMonitor. When provided, the runner feeds
   * every text delta + tool call into it and forwards newly-fired
   * alerts to `sink.appendStreamAlert` (if implemented). When absent,
   * monitoring is disabled — fully back-compat with existing callers.
   *
   * The monitor is shared across iterations within a run; a fresh one
   * is constructed per `run()` if not injected.
   */
...
export class AgentRunner {
  /**
   * Phase 2 — set of StreamMonitor alert types already forwarded to the
   * sink during this run. Each alert type fires `appendStreamAlert` at
   * most once (de-duped by `alert.type`). Lives on the runner instance,
   * not on the monitor, so the runner stays in charge of "exactly once
   * per run" semantics even if the caller injects a shared monitor.
   */
  private emittedAlertTypes = new Set<string>()

  constructor(private readonly deps: AgentRunnerDeps) {}

  /**
   * Phase 2 — return all StreamMonitor alerts fired during this run.
   * Used by agent-loop to roll alerts into the per-run TelemetryRecord.
   * Returns an empty array when no monitor was injected.
   */
  getStreamAlerts(): readonly StreamAlert[] {
    return this.deps.streamMonitor?.getAlerts() ?? []
  }
  ...
  async run(input: AgentRunInput): Promise<void> {
    ...
    while (true) {
      ...
      // D-027 — auto-compact at compaction threshold if compactor present.
      ...
      if (
        this.deps.compact &&
        totalTokens >= (this.deps.compactionThreshold ?? DEFAULT_COMPACTION_THRESHOLD) &&
        !state.compacted
      ) {
  ...
  private async runTurn(
    ...
    const stream = this.deps.adapter.runWithTools(state.messages, this.deps.tools, {
      // D-030 — agent-loop may augment the prompt with /AGENTS.md.
      systemPrompt: this.deps.systemPrompt ?? this.deps.defaultSystemPrompt,
    ...
    for await (const step of stream) {
      switch (step.type) {
        case "text_delta":
          ...
          // Phase 2 — feed the delta to the optional StreamMonitor and
          // forward any newly-fired alerts to the sink. De-duped per run
          // so the same alert type only ever lands on the sink once.
          if (this.deps.streamMonitor) {
            this.deps.streamMonitor.onDelta(step.delta)
            await this.flushNewStreamAlerts(input.messageId)
          }
          break
        ...
        case "tool_call":
          ...
          // Phase 2 — tell the monitor a tool call happened (resets the
          // "no-tool-calls" stall heuristic).
          if (this.deps.streamMonitor) {
            this.deps.streamMonitor.onToolCall()
          }
          break
  ...
  /**
   * Phase 2 — forward any newly-fired StreamMonitor alerts to the sink.
   * Called after each `monitor.onDelta(...)` so alerts surface live (the
   * UI can render them as soon as the runner produces them). The runner
   * tracks emitted alert types in `this.emittedAlertTypes` so the same
   * alert never fires `appendStreamAlert` twice within a single run, even
   * if the underlying regex re-matches across deltas.
   *
   * No-op when the sink doesn't implement `appendStreamAlert` or when no
   * monitor is wired.
   */
  private async flushNewStreamAlerts(messageId: string): Promise<void> {
    const monitor = this.deps.streamMonitor
    if (!monitor) return
    const sink = this.deps.sink
    if (!sink.appendStreamAlert) return

    for (const alert of monitor.getAlerts()) {
      if (this.emittedAlertTypes.has(alert.type)) continue
      this.emittedAlertTypes.add(alert.type)
      await sink.appendStreamAlert(messageId, alert)
    }
  }
}
