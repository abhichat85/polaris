...
describe("StatusDropdown", () => {
  ...
  it("shows only valid transitions for current status", async () => {
    ...
    expect(await screen.findByRole("menuitem", { name: /in progress/i })).toBeInTheDocument()
  ...
  it("from in_progress allows todo, done and blocked", async () => {
    const user = userEvent.setup()
    render(
      <StatusDropdown
    ...
    expect(await screen.findByRole("menuitem", { name: /^todo$/i })).toBeInTheDocument()
  ...
  it("calls updateFeatureStatus mutation when a status is selected", async () => {
    const user = userEvent.setup()
    render(
      <StatusDropdown
        currentStatus="todo"
